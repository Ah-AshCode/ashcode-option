...
class adminOptions
{
	...
	function __construct() {
	    ...
	    add_action( 'admin_head', array($this, 'ash_admin_head') );
	...
	function theme_admin_assets(){
			...
			wp_enqueue_style('ash_chosen', get_stylesheet_directory_uri() . '/admin/assets/css/chosen.css', false, '1.0.0');
			...
			wp_enqueue_script('chosen.proto', get_stylesheet_directory_uri() . '/admin/assets/js/chosen.proto.js', array('jquery'), false, '1.0.0');
	...
	function theme_admin_init_menu(){
		...
		add_menu_page( ADMIN_OPTIONS_TITLE_LABEL, ADMIN_OPTIONS_MENU_LABEL, 'manage_options', ADMIN_OPTIONS_PAGE_SLUG, array($this, 'theme_admin_options_page'), 'dashicons-admin-appearance', 2 );
	...
	function theme_admin_init_menu_toolbar($wp_admin_bar){
		...
		$args = array(
			...
			'href' => admin_url('admin.php?page='.ADMIN_OPTIONS_PAGE_SLUG),
			'meta' => array(
				'title' => ADMIN_OPTIONS_MENU_LABEL
			)
		);
	...
	function theme_admin_options_ui_tabs(){
		...
		foreach($theme_fields as $section){
			...
			$output .= "<li data-tab='{$section['id']}' title='{$section['overview_text']}'>
	...
	function theme_admin_options_ui_content(){
		...
		foreach($theme_fields as $section){
			...
			foreach($section['fields'] as $fields){
				...
				if(!isset($fields['std'])) { $fields['std'] = "";}


				$this->theme_admin_build_fields($fields['type'], $fields['id'], $fields['name'], $fields['class'], $fields['placeholder'], $fields['label'], $fields['desc'], $fields['std'], $options);
	...
	function theme_admin_options_overview(){
		...
		foreach($theme_fields as $section){
				...
				$output .= "<div class='overview-panel col span_1_of_3'><a href='#{$section['id']}'><h1><span class='fa {$section['icon']}'></span>'{$section['name']}'</h1><h2>{$section['overview_text']}</h2></a></div>";
		...
		$output .= "<div class='overview-panel col span_1_of_3'><a href='#export'><h1><span class='fa dashicons-shield'></span></h1><h2>Backup &amp; Restore your settings</h2></a></div>";
	 ...
	 * @return string
	 */
	function theme_admin_build_fields($type="", $id="", $name="", $class="", $placeholder="", $label="", $desc="", $std="", $options, $is_meta="", $opt_ash= ""){
		...
		case "select":
		 ...
		 echo "<div class='form-row type-radio-image'><h4 class='ash_option_title'>$label</h4>";

			
		
  
		foreach(  $options as $option => $scr){

			 if( $this->theme_opt($name) === $option ){$checked = "checked=\"checked\"";  } else{$checked = "";} 

			echo "<input id=' " . $option . "' type='radio' name='" . $field_name . "[{$name}]' value='" . $option . "' data-image='" . $scr . "' class='radio-images' ". $checked ." />
			      <label for='". $option ."'> $option </label>";

				}

					echo "<div class='field-desc'>$desc</div></div>";
			...
			case "on-off":
              ...
              if( $this->theme_opt($name) === '1' ){$checked = "checked=\"checked\"";  } else{$checked = "";} 

             if($is_meta){
				echo "<p><label>$label</label></p>";

			}else{
				echo "<div class='form-row'><h4 class='ash_option_title'>$label</h4>";

			}
				

				echo "<input type='checkbox' name='" . $field_name . "[{$name}]' value='1'  id='" . $id .  "' class='lcs_check' ". $checked."> ";
		...
		case "font":
		    ...
		    if($is_meta){
				...
				if (file_exists(get_template_directory() . "/admin/assets/libs/google-fonts.json")) {
					...
					echo "<div class='form-row panel'>";
					echo "<div class='panel-heading clickable_".$id."'>
                    <h3 class='panel-title'> $label</h3>
                    <span class='pull-right clickable_".$id."'>
                    <i class='fa fa-plus'></i>
                    </span>";
					echo "</div>";
					echo "<div class='ash_panel ash-panel-content_" . $id . "'>";
					echo " <label>";
					echo "<div class='field-desc'>$desc</div>";
					echo "<div class='select'><select name='" . $field_name . "[{$name}]' id='{$id}' class='gfont-select {$class}' placeholder='{$placeholder}'>";
					...
					foreach ($options as $option => $value) {
					   ...
					   if ($option == 'font_size' && $value == true) {
							...
							echo "  <input type='number' size='1' name='" . $field_name . "[{$name}_size]' value='{$this->theme_opt($name."_size",$args)}' /><span class='input-addon'>px</span>";

						}
						if ($option == 'line_height' && $value == true) {

							echo "  <input type='number' size='1' name='" . $field_name . "[{$name}_height]' value='{$this->theme_opt($name."_height",$args)}' /><span class='input-addon'>px</span>";
					...
					echo "</label>";
					?>
					<div style="border: 1px solid #ddd; margin: 20px 0 0;padding: 20px; background: #f8f8f8;width: 386px; margin-left: 176px;" class="testtextbox" id="<?php echo $id; ?>">The Quick Brown Fox Jumps Over The Lazy Dog. 1234567890</div>
<script type="text/javascript">
	jQuery(document).ready(function() {
  function accordion() {

    var $this = jQuery(this);
    var $thisNext = jQuery(".ash-panel-content_<?php echo $id; ?>");
    var $thisParent = $this.parent();

    // If accordion__content is hidden
    if ($thisNext.is(':hidden')) {
      // Go back up to parent, find the class with accordion__header--active, remove classes, then slide up the content
      $thisParent.find('.accordion__header--active')
        .removeClass('accordion__header--active')
        .next()
        .slideUp();
      // Then slide down the content of whichever header is clicked
      $thisNext.slideDown();
      // Add active class to header 
      $this.addClass('accordion__header--active');

    } else {
      // If it's not hidden, slide up the content
      $thisNext.slideUp();
  ...
  jQuery(".panel").on("click", ".panel-heading .clickable_<?php echo $id; ?>", accordion);
});

jQuery(document).ready(function($){
jQuery("select#<?php echo $id; ?>").change(function(){
var val = jQuery("select#<?php echo $id; ?>").val();
//var valx = val.replace(/ /g, "+");
jQuery("#cFontStyleWColor11").text('#<?php echo $id; ?> { font-size: 16px; font-family: "'+ val +'" !important; }');
				...
				if(!empty($fields['css']) && !empty($this->theme_opt($fields['name']))){
					...
					switch($fields['css']['property']){
						...
						case "font":
                            ...
                            $font_height = $this->theme_opt($fields['name']."_height");
							...
							if(!empty($font_height)) {
								$css .= "line-height: {$font_height}px !important;";
							}
