 ...
 *
 */
class ASHFramework_Taxonomy extends ASHFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'ash_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && ASH_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = ash_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $ash_errors = get_transient( 'ash-taxonomy-transient' );

    wp_nonce_field( 'ash-taxonomy', 'ash-taxonomy-nonce' );

    echo '<div class="ash-framework ash-taxonomy ash-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo ash_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( ash_get_var( 'ash-taxonomy-nonce' ), 'ash-taxonomy' ) ) {

      $errors = array();
      $taxonomy = ash_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'ash_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'ash_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'ash_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'ash_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'ash_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'ash-taxonomy-transient', $errors, 10 );
