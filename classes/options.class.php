 ...
 *
 */
abstract class ASHFramework_Options extends ASHFramework_Abstract {
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="ash-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'ASH_OPTIONS_DEBUG' ) && ASH_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'ash-framework' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". esc_html__( 'USAGE', 'ash-framework' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "ash_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'ash-framework' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'ash_OPTIONS_DEBUG_LIGHT' ) && ash_OPTIONS_DEBUG_LIGHT ) ) {
      ...
      $out .= ( isset( $this->field['id'] ) ) ? "ash_get_option( '". $this->field['id'] ."' );" : '';
      $out .= "\n";
      $out .= "<strong>". esc_html__( 'ID', 'ash-framework' ) .":</strong>";
      $out .= "\n";
  ...
  public function element_get_error() {
    ...
    if( ! empty( $ash_errors ) ) {
      foreach ( $ash_errors as $key => $value ) {
        if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
          $out .= '<p class="ash-text-warning">'. $value['message'] .'</p>';
        }
      }
    }
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="ash-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
  ...
  public function element_after_multilang() {
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      $out .= '</fieldset>';
      $out .= '<p class="ash-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'ash-framework' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? ash_language_defaults() : false;
  }

}

// load all of fields
ash_load_option_fields();
